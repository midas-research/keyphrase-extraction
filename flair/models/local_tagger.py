...
class SequenceTaggerFast(flair.nn.Model):
    def __init__(
            self,
            hidden_size: int,
        ...
        super(SequenceTaggerFast, self).__init__()
    ...
    def forward(self, sentences: List[Sentence]):
        ...
        for s_id, sentence in enumerate(sentences):
            ...
            sentence_tensor = torch.cat([token.get_embedding().unsqueeze(0) for token in sentence], 0)

            if self.relearn_embeddings:
                sentence_tensor = self.embedding2nn(sentence_tensor)

            token_embeddings.append(sentence_tensor)
        ...
        if self.use_rnn:
            packed = torch.nn.utils.rnn.pack_sequence(token_embeddings, lengths)

            rnn_output, hidden = self.rnn(packed)

            batch_tensor, output_lengths = torch.nn.utils.rnn.pad_packed_sequence(
                rnn_output
            )

            if self.use_dropout > 0.0:
                sentence_tensor = self.dropout(batch_tensor)
            # word dropout only before LSTM - TODO: more experimentation needed
            # if self.use_word_dropout > 0.0:
            #     sentence_tensor = self.word_dropout(sentence_tensor)
            if self.use_locked_dropout > 0.0:
                sentence_tensor = self.locked_dropout(batch_tensor)

        # print(sentence_tensor.size())
        # asd

        features = self.linear(batch_tensor)
