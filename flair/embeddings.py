...
from flair.CharCNN import CharCNN 
import gensim
import numpy as np
import torch
...
from flair.SpanBert.tokenization import BertTokenizer as SpanBertTokenizer
...
class StackedEmbeddings(TokenEmbeddings):
    ...
    def embed(
        ...
        if type(sentences) is Sentence:
            sentences = [sentences]
       # print("asdfasfasf:",self.__embedding_length)
...
class CharacterCNNEmbeddings(TokenEmbeddings):
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]):
        ...
        for sentence in sentences:
            ...
            longest_token_in_sentence = max(chars2_length)
            tokens_mask = torch.zeros(
                (len(tokens_sorted_by_length), self.max_length),
                dtype=torch.long,
                device=flair.device,
            )

            for i, c in enumerate(tokens_sorted_by_length):
                c = c[:min(len(c),self.max_length)]
                tokens_mask[i, : chars2_length[i]] = torch.tensor(
                    c, dtype=torch.long, device=flair.device
                )
            # chars for rnn processing
            chars = tokens_mask
            character_embeddings = self.char_cnn(self.char_embedding(chars))
            for token_number, token in enumerate(sentence.tokens):
                token.set_embedding(self.name, character_embeddings[token_number])
...
class SpanBertEmbeddings(TokenEmbeddings):
    def __init__(
        self,
        spanbert_path,
        bert_model_or_path: str = "bert-base-cased",
        layers: str = "-1,-2,-3,-4",
        pooling_operation: str = "first",
        use_scalar_mix: bool = False,
    ):
        ...
        self.model = BertForMaskedLM.from_pretrained(
                 pretrained_model_name_or_path=spanbert_path,
                 output_hidden_states=True,
                 )
        self.tokenizer = BertTokenizer.from_pretrained(bert_model_or_path)
       # self.tokenizer = SpanBertTokenizer("/Users/r17935avinash/Downloads/spanbert_hf")
        self.layer_indexes = [int(x) for x in layers.split(",")]
        self.pooling_operation = pooling_operation
        self.use_scalar_mix = use_scalar_mix
        self.name = str(bert_model_or_path)
